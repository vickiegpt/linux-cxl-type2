...
/* Module params for synthetic RCH CXL memory window */
static u64 rch_hpa_base = 0x180000000000ULL;
...
struct rch_dport_check {
	int port_id;
	bool found;
};

static int check_dport_has_decoder(struct device *dev, void *data)
{
	struct rch_dport_check *chk = data;
	...
	d = to_cxl_decoder(dev);
	for (j = 0; j < d->interleave_ways; j++) {
		if (d->target_map[j] == chk->port_id) {
			chk->found = true;
			return 1;
		}
	}
...
/**
 * cxl_inject_synthetic_cfmws - Create synthetic root decoders for RCH dports
 *                              that have no CFMWS coverage in ACPI CEDT.
 * @root_port: CXL root port
					...
					struct device *host)
{
	...
	xa_for_each(&root_port->dports, index, dport) {
		struct rch_dport_check chk = { .port_id = dport->port_id };
		struct cxl_root_decoder *cxlrd;
		struct cxl_decoder *cxld;
		struct resource *res;
		int rc;
		...
		/* Check if any existing root decoder already targets this dport */
		device_for_each_child(&root_port->dev, &chk,
				      check_dport_has_decoder);
		if (chk.found) {
			dev_dbg(host, "RCH dport %d already has root decoder\n",
				dport->port_id);
			continue;
		}
		...
		if (!res) {
			dev_err(host, "Failed to alloc CXL resource for RCH\n");
			return;
		}

		rc = add_or_reset_cxl_resource(cxl_res, res);
		if (rc) {
			dev_err(host, "Failed to add CXL resource for RCH: %d\n", rc);
			return;
		}

		cxlrd = cxl_root_decoder_alloc(root_port, 1);
		if (IS_ERR(cxlrd)) {
			dev_err(host, "Failed to alloc root decoder for RCH: %ld\n",
				PTR_ERR(cxlrd));
			return;
		}
		...
		rc = cxl_decoder_add(cxld);
		if (rc) {
			dev_err(host, "Failed to add synthetic root decoder: %d\n", rc);
			put_device(&cxld->dev);
			return;
		}
		...
		if (rc) {
			dev_err(host, "Failed autoremove for synthetic root decoder: %d\n", rc);
			return;
		}
...
static int cxl_acpi_probe(struct platform_device *pdev)
{
	...
	/* Inject synthetic root decoders for RCH dports without CFMWS */
	cxl_inject_synthetic_cfmws(root_port, cxl_res, &ctx, host);
