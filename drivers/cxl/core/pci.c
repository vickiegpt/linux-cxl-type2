			...
			struct cxl_endpoint_dvsec_info *info)
{
	...
	for (i = 0; i < hdm_count; i++) {
		...
		/* Skip ranges where base wasn't programmed (config_lock) */
		if (!base) {
			dev_dbg(dev, "DVSEC Range%d base not programmed, skipping\n", i);
			continue;
		}
			...
			struct cxl_endpoint_dvsec_info *info)
{
	...
	/*
	 * No DVSEC ranges (e.g. base not programmed due to config_lock).
	 * Skip range validation and just enable HDM decoder directly.
	 */
	if (!info->ranges) {
		rc = devm_cxl_enable_hdm(&port->dev, cxlhdm);
		if (rc)
